...
$sql = "SELECT lat,lng FROM cache WHERE (lat between " . $_POST['lat0'] . " and " . $_POST['lat1'] . ") and (lng between " . $_POST['lng0'] . " and " . $_POST['lng1'] . ")";
$result = $db->execute($sql);

$points = array();
while($myrow = mysql_fetch_object($result)){
  $points[] =  array("location" => array($myrow->lat, $myrow->lng));
}
//var_dump($points);
/*if((int)$_POST['radius'] < 2){
 // $_POST['radius'] = 2;
}
if((int)$_POST['zoom'] < 7){
  $_POST['radius'] = 3000;
...
$cluster = new Cluster();
$clusterPoint = $cluster->createCluster($points,(int)$_POST['radius'],(int)$_POST['zoom'], 0);

echo json_encode($clusterPoint);
