<?php
  require_once "config.php";	
	...
	if ($action=='delete') 
	{
		$sql = "DELETE FROM forum WHERE documentid = $docid";
	...
	if ($action=='edit') 
	{
		$sql = "SELECT title, content, image FROM forum WHERE documentid = $docid";
		$result = $db->execute($sql);
		while ($myrow = mysql_fetch_object($result))
		{
			$TITLE = $myrow->title;
			$CONTENT = iconv("UTF-8","windows-1251",$myrow->content);
			$IMAGE = $myrow->image;
		}
	}
	else
	...
	{	
		...
		else
		{
			if ($action=='update')
			{	
				$sql = "UPDATE forum SET title='$TITLE', content='$CONTENT', image='$IMAGE' WHERE documentid = $docid";
				$db->execute($sql, false);
			...
			{			
				...
				$sql = "INSERT INTO forum (documentid, title, content, image) VALUES ($lastid, '$TITLE', '$CONTENT', '$IMAGE')";
				$db->execute($sql, false);
				
				$sql = "INSERT INTO alldocs (sid, documentid, position, title, doctype, stid) VALUES ($sid, $lastid, $lastid, '$TITLE', 'forum', $stid)";
				$db->execute($sql, false);
...
<html>
<head>
	<title>Создание/редактирование темы форума</title>
...
<tr>
	...
	<td class="top2" valign="middle"><b>С</b>оздание/редактирование темы форума</td>
...
?>
		...
		<tr>
			...
			<td></td>
			<td><input type="text" name="IMAGE" class="inp" value="<?php echo $IMAGE; ?>"/></td>
		</tr>
		<tr height="100%">
